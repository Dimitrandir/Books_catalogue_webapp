...
2. Снимката се изпраща към Claude API (vision) с prompt да върне JSON:
   `{title, authors: [...], publisher}`. Без ISBN — на предната корица на
   българските книги обикновено не е отпечатан (пази се отзад/на
   копирайт страницата, каквато не се снима).
3. Заявка по `title`+`authors` към Open Library API (или Google Books API
   като fallback) за: резюме, година.
